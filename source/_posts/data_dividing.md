...
- Actually, to estimate
  ...
  - $\forall x \in \mathbb{R},round_1(x)=\begin{cases}
    ...
    \end{cases}$ in [analyses of round
    function](https://little-train.com/posts/34195fcb.html),
  - or, $\forall x \in \mathbb{R},~round_2(x)=
    \begin{cases}
     \lfloor x\rfloor,\text{if }x-\lfloor x\rfloor<0.5 \\
...
- Second, consider the range $F_n=\{f(r,n,N)|(r,N)\in D_{r,N}\}$, if we
  ...
  - The helper function $I(x,k):\{D,\mathbb{Z}\}\rightarrow \mathbb{R}$,
...
with the solution that based on $floor(\cdot)$ function.
...
\$n,N ^+, n N \$, conside
$D_y = \{y|y=[y_1,y_2,\ldots,y_n]\in \mathbb{N}^n,\sum_{i=1}^{n}y_i=\|y\|_1=N\}$.
Except $\empty$, how many elements in $D_y$?
...
1.  Preliminary illustrations:
    ...
    So, there are
    $D_b\subseteq D^{'}_b, \empty \ne D^{*}_b\subseteq D_b,~\empty\ne D^{*'}_b \subseteq D^{'}_b$
    ...
    - If \$h, \$
      ...
      - Obviously, $B_3 \ne \empty$.
        ...
        - The
          ...
          $\empty\ne D^{*'}_b \subseteq D^{'}_b$ has been illustrated in
          `1`.
...
[^4]: (2022, November 17). Built-in Functions — Python 3.11.0 documentation. Docs. https://docs.python.org/3/library/functions.html?highlight=sorted#sorted

[^5]: (2022, November 17). Sorting algorithm - Wikipedia. En. https://en.wikipedia.org/wiki/Sorting_algorithm#Stability
